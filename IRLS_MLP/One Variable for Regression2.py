...
# iteration number
iter_num = 5000
# define batch_size and input_dim for the neural network
batch_size = 40
input_dim = 1
#%% make pure data
...
# add hidden layer-1
l1 = add_layer(xs, input_dim, 20, n_layer=1, activation_function=tf.nn.relu)
l2 = add_layer(l1, 20, 10, n_layer=2, activation_function=tf.nn.relu)
# add output layer
prediction = add_layer(l2,10 , 1, n_layer=4, activation_function=None)
    ...
    with tf.Session() as sess:        
        ...
        for i in range(iteration_num):
            ...
            if i % 8 == 0 and i!=0:
                ...
                res_std  = np.std(res)
                ave_true_loss = np.mean((pre-true_y)**2)
...
#%%
pre_pure_None,sw_PuNo,loss_pu_None = training_process(input_data=x_p,output_data=y_p,true_y=y_p,IRLS=None,iteration_num=iter_num)
...
plt.title('Training without IRLS using noisy data loss: %f'%loss_sw_None)

# plot model trained with noisy data and IRLS
ax = fig.add_subplot(424)
plt.scatter(x,pre_noise_IRLS,c='r',s=3,label='predict',hold = 'True')
plt.scatter(x_data,y_data,c='b',s=3,label='train',hold = 'True')
plt.legend(loc='lower left')
plt.title('Training with IRLS using noisy data loss: %f'%loss_sw_IRLS)
...
plt.scatter(x_p,pre_pure_None,c='r',s=3,label='predict')
plt.scatter(x_p,y_p,c='b',s=3,label='train')
plt.legend(loc='lower left')
plt.title('Training without IRLS using pure data: %f'%loss_pu_None)
...
ax = fig.add_subplot(424)
plt.scatter(x_data[z_idx],y_data[z_idx],c='g',s=5,hold = 'True',label='res>std')
plt.legend(loc='lower left')
#%%
#s_index = []
#s_num = 0
